...
docker build --pull \
       --tag $APP_PUBLIC_DOCKER_COMMIT_TAG \
       --tag $APP_PUBLIC_DOCKER_BRANCH_TAG \
       --tag $APP_PRIVATE_DOCKER_COMMIT_TAG \
       --tag $APP_PRIVATE_DOCKER_BRANCH_TAG \
       -f docker/Dockerfile .
...
## deploy tool easier.
docker push $APP_PUBLIC_DOCKER_BRANCH_TAG
docker push $APP_PUBLIC_DOCKER_COMMIT_TAG
docker push $APP_PRIVATE_DOCKER_BRANCH_TAG
docker push $APP_PRIVATE_DOCKER_COMMIT_TAG
